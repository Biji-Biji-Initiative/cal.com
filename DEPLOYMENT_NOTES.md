...
- Secret: `calcom-env` created from `env-vars-production.yaml` (not committed); applied to namespace `calcom`.
...
## Credentials
- Production secrets live in `env-vars-production.yaml`; do not commit. Recreate `calcom-env` K8s secret from that file when needed. Cloudflare token secret is created in `cert-manager` namespace for DNS-01.
